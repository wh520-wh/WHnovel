...
from app.api import chat_options
...
DEFAULT_TAIL_PAYLOAD = {
    ...
    'story_state': {'chapter': '第一章', 'progress': 12, 'current_goal': '', 'current_conflict': ''},
...
def test_start_stream_emits_delta_before_tail_and_persists_after_validation(monkeypatch):
    ...
    monkeypatch.setattr(chat_api, '_stream_model_once', _build_fake_stream([
        '这是实时',
        '流式回复',
    ]))
...
def test_options_generation_rejects_concurrent_duplicate_request_logic_contract():
    ...
    lock = _get_or_create_lock(chat_options._option_generation_locks, chat_options._option_generation_locks_guard, archive['id'])
...
def test_send_stream_switches_backup_only_before_first_delta(monkeypatch):
    ...
    resp = client.post("/api/chat/send-stream", json={"archive_id": archive_id, "message": "继续推进剧情"})
    assert resp.status_code == 200
...
def test_stream_second_call_fails_draft_persisted(monkeypatch):
    ...
    resp = client.post('/api/chat/send-stream', json={'archive_id': archive['id'], 'message': '推进剧情'})
    assert resp.status_code == 200
    assert 'event: error' in resp.text
...
def test_stream_second_call_returns_invalid_json(monkeypatch):
    ...
    resp = client.post('/api/chat/send-stream', json={'archive_id': archive['id'], 'message': 'test'})
    assert resp.status_code == 200
    assert 'event: error' in resp.text
...
def test_stream_body_pollution_before_first_delta_switches_backup(monkeypatch):
    ...
    resp = client.post("/api/chat/send-stream", json={"archive_id": archive_id, "message": "继续推进"})
    assert resp.status_code == 200
