...
def test_options_generate_retry_returns_retry_result():
    ...
    with (
        patch.object(chat_router, "_acquire_option_generation_lock", _fake_lock),
        patch.object(chat_router, "_get_or_create_settings") as mock_settings,
        patch.object(
            chat_router, "_build_messages", return_value=[{"role": "user", "content": "test"}]
        ),
        ...
        patch.object(
            chat_router, "_call_ai_with_failover", side_effect=[first_result, retry_result]
        ),
        patch.object(
            chat_router,
            "validate_options_list",
            side_effect=[(False, "选项含模糊词"), (True, None)],
        ),
    ):
